...
class DBProvisioner(object):
    ...
    def provision_mariadb(self, info: DBInfo):
        self.logger.info("Connecting to '{}' database  as user '{}'".format(info.connect_db_name, info.master_username))
        try:
            connection = pymysql.connect(
                host=info.host,
        ...
        if info.provision_user:
            ...
            if info.provision_user in usernames:
                ...
                cursor.execute(query)
                query = "GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, PROCESS, REFERENCES, INDEX, ALTER, SHOW DATABASES, EXECUTE, CREATE VIEW, SHOW VIEW, CREATE ROUTINE, ALTER ROUTINE, CREATE USER, EVENT, TRIGGER ON * . * TO '{}'@'%';".format(info.provision_user)
                cursor.execute(query)
        ...
        if info.provision_db_name in databases_names:
            ...
            if info.provision_user:
                ...
                query = "GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, REFERENCES, INDEX, ALTER, EXECUTE, CREATE VIEW, SHOW VIEW, CREATE ROUTINE, ALTER ROUTINE, EVENT, TRIGGER ON {} . * TO '{}'@'%';".format(
                    info.provision_db_name,
                    info.provision_user,
                )
                cursor.execute(query)
    ...
    def provision(self):
        ...
        if engine == 'postgres':
            ...
            self.provision_mariadb(db_info)
        else:
